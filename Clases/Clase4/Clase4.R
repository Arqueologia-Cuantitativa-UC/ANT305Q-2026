# Clase 4. Categorías, porcentajes y gráficos de barra.
...
# Para calcular qué porcentaje representa cada fila dentro de su columna usamos la función prop.table(), que calcula proporciones, y luego multiplicamos por 100 para obtener porcentajes. Primero transformamos nuestra tabla a un data.frame, ya que vamos a agregar columnas nuevas con los porcentajes calculados.
...
# Para poder graficar en ggplot2 necesitamos que los datos estén en un formato específico: cada fila debe ser una observación. Por eso creamos un nuevo data.frame con tres columnas: Sitio (zona geográfica), Categoria (tipo de cerámica) y Porcentaje (el valor calculado). Usamos la función rep() para repetir los nombres de las zonas, y escribimos las categorías y porcentajes en el mismo orden.
...
# 1. Crea un nuevo data.frame a partir de Liticos y calcula los porcentajes de columna para cada zona geográfica.
...
# 4. Realiza un boxplot con Largo liticos para comparar cómo se distribuyen los largos de las puntas de obsidiana por zona geográfica. ¿Dónde hay puntas más largas? ¿Dónde hay puntas más pequeñas? ¿Las distribuciones son simétricas o dispersas? ¿Dónde tienden a agruparse los datos en cada boxplot?
